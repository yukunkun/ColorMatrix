...
public class DownLoadUtils {
    ...
    public static long download(Context context, String url,String path, boolean isGif){
...
//        request.setDestinationUri(Uri.fromFile(new File(Environment.getExternalStorageDirectory()+"/moral/", System.currentTimeMillis()+".mp4")));
        if(isGif){
            request.setDestinationInExternalPublicDir(path, System.currentTimeMillis()+".gif" );
        }else {
            request.setDestinationInExternalPublicDir(path, System.currentTimeMillis()+".png" );
        }
