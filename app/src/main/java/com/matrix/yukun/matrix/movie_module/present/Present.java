 ...
 */
public class Present implements BasePresentImpl {
    Top250Fragment mView;
    CompositeSubscription compositeSubscription = new CompositeSubscription();

    public Present(Top250Fragment mainActivity) {
        this.mView=mainActivity;
    }

    public void getInfo(int start){
        Retrofit retrofituil = RetrofitApi.getInstance().retrofituil();
        retrofituil.create(MovieService.class).getTopMovie(start,10).enqueue(new Callback<HttpResult<Subjects>>() {
            @Override
...
//        Subscription subscribe = RetrofitUtils.getMovie(start, 10).subscribe(new Subscriber<List<Subjects>>() {
//            @Override
