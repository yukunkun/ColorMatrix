...
import com.matrix.yukun.matrix.tool_module.gif.utils.bitmap.FastYUVtoRGB;
...
public class GifExtractor {
    ...
    private GifEncoderListener mGifEncoderListener;
    ...
    public void addEncolerListener(GifEncoderListener gifEncoderListener){
        mGifEncoderListener=gifEncoderListener;
    }
    ...
    public void encoder(final String gifPath, final long begin, final long end, final int fps,final int speed, final int gifWidth, final int gifHeight){
        ...
        if(mGifEncoderListener!=null){
            mGifEncoderListener.startEncoder();
        }
        Thread thread = new Thread(){
            ...
            public void run() {
                            ...
                            if(time >= begin && time <= endTime){
                                if(time >= startTime){
                                    ...
                                    if(gifWidth != -1 && gifHeight != -1){
                                        ...
                                        bitmap = ImageUtils.resizeImage(bitmap, gifWidth, gifHeight);
                                    }else{
                                    ...
                                    startTime += frameTime;
                                    if(mGifEncoderListener!=null){
                                        mGifEncoderListener.progressEncoder(100,p);
                                    }
                                }
                            }
                ...
                if(mGifEncoderListener!=null){
                    mGifEncoderListener.endEncoder();
                }
