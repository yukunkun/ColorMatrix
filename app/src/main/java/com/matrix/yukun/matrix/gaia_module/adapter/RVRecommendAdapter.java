...
public class RVRecommendAdapter extends BaseQuickAdapter<GaiaIndexBean,BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, GaiaIndexBean item) {
        ...
        helper.setText(R.id.player_rec_author,item.getNickName());
        if(item.getTime()!=null){
            helper.getView(R.id.player_rec_cover).setVisibility(View.VISIBLE);
            helper.setText(R.id.player_rec_publish_time,DataUtils.getGaiaTime(item.getTime().getTime()));
        }else {
            helper.getView(R.id.player_rec_cover).setVisibility(View.GONE);
        }
        ImageView cover=(ImageView) helper.getView(R.id.player_rec_cover);
