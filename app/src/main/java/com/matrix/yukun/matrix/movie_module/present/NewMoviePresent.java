 ...
 */
public class NewMoviePresent implements BasePresentImpl{
    NewMovieFragment mView;
    CompositeSubscription compositeSubscription = new CompositeSubscription();

    public NewMoviePresent(NewMovieFragment mainActivity) {
        this.mView=mainActivity;
    }

    public void getInfo(int start){
        Retrofit retrofituil = RetrofitApi.getInstance().retrofituil();
        retrofituil.create(MovieService.class).getNewMovie(start,10).enqueue(new Callback<HttpResult<Subjects>>() {
            @Override
...
//        Subscription subscribe = RetrofitUtils.getNewMovie(start, 10).subscribe(new Subscriber<List<Subjects>>() {
//            @Override
