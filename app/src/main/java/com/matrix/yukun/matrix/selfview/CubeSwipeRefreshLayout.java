...
public class CubeSwipeRefreshLayout extends ViewGroup {
    ...
    public CubeSwipeRefreshLayout(Context context) {
        this(context, null);
    }

    @SuppressWarnings("deprecation")
    public CubeSwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SuperSwipeRefreshLayout);

        //自定义属性设置下拉头背景高度
        int HEADER_VIEW_HEIGHT = ta.getInteger(R.styleable.SuperSwipeRefreshLayout_header_view_height, 50);
        //自定义属性设置默认下拉头空白背景高度
        int DEFAULT_CIRCLE_TARGET = ta.getInteger(R.styleable.SuperSwipeRefreshLayout_default_circle_target, 50);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mRefreshing) {
            mHeadViewContainer.layout((width / 2 - headViewWidth / 2), mCurrentTargetOffsetTop + mChatTitleBarHeight, (width / 2 + headViewWidth / 2), mCurrentTargetOffsetTop + headViewHeight + mChatTitleBarHeight);// 更新头布局的位置
        }
        else {
