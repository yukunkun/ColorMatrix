...
public class EmoticonManager {
    ...
    // max cache size
    private static final int CACHE_MAX_SIZE = 6 * 1024 * 1024;
    ...
    private static class Entry {
        ...
        Entry(String text, String assetPath, String gifPath) {
            this.text = text;
    ...
    public static Drawable getDisplayDrawable(Context context, int index) {
        String text = (index >= 0 && index < defaultEntries.size() ? defaultEntries.get(index).text : null);
    ...
    public static String getAssetPath(int index) {
        return index >= 0 && index < defaultEntries.size() ? defaultEntries.get(index).gifPath : null;
    }
    ...
    public static String getGifPath(String text) {
        Entry entry = text2entry.get(text);
        if (entry == null) {
            return null;
        }
    ...
    private static class EntryLoader extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            ...
            if (localName.equals("Catalog")) {
                ...
                String gifName = attributes.getValue(uri, "Gif");
                if (!TextUtils.isEmpty(gifName)) {
                    gifName = EMOJI_DIR + catalog + "/" + gifName;
                }
                else {
                    gifName = null;
                }
                Entry entry = new Entry(tag, EMOJI_DIR + catalog + "/" + fileName, gifName);
