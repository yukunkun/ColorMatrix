...
public class WaterLoadView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.WaterLoadView_circleColor) {
                mCircleColor = a.getColor(attr, Color.WHITE);

            } else if (attr == R.styleable.WaterLoadView_circleSmallRadio) {
                mCircleSmallRadio = a.getInt(attr, 15);

            } else if (attr == R.styleable.WaterLoadView_circleTop) {
                mCircleTop = a.getInt(attr, 70);

            } else if (attr == R.styleable.WaterLoadView_loadTime) {
