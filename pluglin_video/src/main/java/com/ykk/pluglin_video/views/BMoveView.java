...
public class BMoveView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.BMoveView_circleColor) {
                mCircleColor = a.getColor(attr, Color.WHITE);

            } else if (attr == R.styleable.BMoveView_lineColor) {
                mLineColor = a.getColor(attr, Color.GRAY);

            } else if (attr == R.styleable.BMoveView_circleCenterColor) {
                mCircleCenterColor = a.getColor(attr, Color.GRAY);

            } else if (attr == R.styleable.BMoveView_lineDuration) {
                mLineDuration = a.getInt(attr, 500);

            } else if (attr == R.styleable.BMoveView_lineWidth) {
                mLineWidth = a.getInt(attr, 5);

            } else if (attr == R.styleable.BMoveView_circleDuration) {
                mCircleDuration = a.getInt(attr, 500);

            } else if (attr == R.styleable.BMoveView_circleRadio) {
