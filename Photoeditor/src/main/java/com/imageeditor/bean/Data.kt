...
data class InputTextData(val id: String?, val text: String?, val color: Int?, val bgcolor: Int?) : SharableData
 ...
 * @property editorPath the image you wanted to edit . 你想要编辑的图片
 * @property editor2SavedPath in which path edit image to save. 编辑图片保存的路径
 */
data class EditorSetup(val originalPath: String?, val editorPath: String?, val editor2SavedPath: String,val isP2P:Boolean) : SharableData
 ...
 */
data class EditorResult(val originalPath: String?, val editorPath: String?, val editor2SavedPath: String, val editStatus: Boolean,val isSave:Boolean) : SharableData
 ...
 */
data class TextPastingSaveState(val text: String, val textColor: Int, val bgtextColor: Int, val initTextRect: RectF, private val initDisplay: RectF, private val display: Matrix) : PastingSaveStateMarker(initDisplay, display) {
    override fun deepCopy(): SaveStateMarker {
        val state = TextPastingSaveState(text, textColor,bgtextColor, RectF(initTextRect), RectF(initDisplay), Matrix(display))
        state.id = this.id
        return state
    }
